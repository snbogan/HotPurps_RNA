...
ggplot(data = larv_morph_df,
       aes(x = Mat, y = area_mm2, group = bucket)) +
  facet_grid(~stage) +
  geom_boxplot() +
  geom_point() +
...
glm1s <- dlply(tab_prism_df, c("geneid"), function(df) 
glm.nb(CPM ~ 1 + Mat + Dev, data = df))
...
nrow(filter(sig_mat_meth_df, X2.5. > 0)) #206 are positive

nrow(filter(sig_mat_meth_df, X2.5. < 0)) #522 are negative
