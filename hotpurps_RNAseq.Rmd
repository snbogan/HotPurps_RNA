...
input="mat_gomwu_input.csv" # two columns of comma-separated values: gene id, continuous measure of significance. To perform standard GO enrichment analysis based on Fisher's exact test, use binary measure (0 or 1, i.e., either sgnificant or not).
...
In 04/11/2024 meeting:

- JC mentioned expectation that dev indirect effects be enriched by biomineralization-related GOs
- SB mentioned expectation that mat indirect effects are more associated with adaptive phenotypic changes

Plot ideas
- Regraph of Jannine's prism data?
- MD plots for DE (2 plots) and SEMs (4 plots: 2 treatments x 2 phenotypes)
- Correlations between abs logFC and indirect, phenotypic effects
- GO enrichment chart
...
mat_mediations <- parLapply(cl, 1:length(lm1s), run_mediation)
...
Developmental mediations

```{r}

# Function to perform maternal mediation analysis
run_mediation_dev <- function(index) {
  tryCatch({
    summary(mediate(lm1s[[index]], lm2s[[index]], 
                     boot = TRUE, sims = 10, 
                     treat = "Dev", mediator = "value"))
  }, error=function(e){paste("Error in index", index, ":", e$message)}) # Return the error message
}
...
cl <- makeCluster(no_cores)
clusterEvalQ(cl, library(mediation))
clusterExport(cl, varlist = c("lm1s", "lm2s", "run_mediation_dev"))

# Run mediation in parallel
mat_mediations_dev <- parLapply(cl, 1:length(lm1s), run_mediation_dev)

# Stop the cluster
stopCluster(cl)

names(mat_mediations_dev) <- names(lm1s)

# Save and reload mediation
save(mat_mediations_dev, file = "04182024_dev_mediations.Rdata")

load("04182024_dev_mediations.Rdata")
...
ind_est_df$column_label = rownames(ind_est_df)
...
ind_est_df$geneid <- ind_est_df$column_label
...
write.csv(na.omit(data.frame(geneid = paste("rna-", ind_est_df$transcript, sep = ""),
                     coef = ind_est_df[,6])),
          file = "~/Documents/GitHub/HotPurps_RNA_04032024_backup/GO_MWU-master/mat_ind_gomwu_new.csv",
          row.names = FALSE) # Resave the csv inexcel before GOMWU run

# Export gomwu df of coefficients
write.csv(na.omit(data.frame(geneid = paste("rna-", ind_est_df$transcript, sep = ""),
                     coef = ind_est_df[,2])),
          file = "~/Documents/GitHub/HotPurps_RNA_04032024_backup/GO_MWU-master/mat_ind_signed_p.csv",
          row.names = FALSE) # Resave the csv inexcel before GOMWU run
...
nrow(filter(ind_p, fdr < 0.05)) # 1147; 17% of all genes gave mediated fitness effect)
nrow(filter(ind_p, fdr < 0.05)) / nrow(ind_p)

# Filter to include only transcripts with significant mediation parameter estimate
sig_mat_meth_df <- filter(ind_ci, X2.5. > 0 & X97.5. > 0 | X2.5. < 0 & X97.5. < 0) # 1375 mat indirect genes
nrow(sig_mat_meth_df) # 1375

# 7.98% of genes has mediated fitness effect
nrow(sig_mat_meth_df) / nrow(ind_p)

# How many mediation effects are positive vs. negative
nrow(filter(sig_mat_meth_df, X2.5. > 0)) #483 are positive
...
for (i in 1:length(mat_mediations_dev)) {
  if (is.list(mat_mediations_dev[[i]]) || is.data.frame(mat_mediations_dev[[i]])) {
    if (!is.null(mat_mediations_dev[[i]]$d0.p)) {
      ind_p_dev[[i]] <- mat_mediations_dev[[i]]$d0.p
    } else {
...
names(ind_p_dev) <- names(mat_mediations_dev)
...
for (i in 1:length(mat_mediations_dev)) {
  if (is.list(mat_mediations_dev[[i]]) || is.data.frame(mat_mediations_dev[[i]])) {
    # Create a one-row data frame from the confidence intervals
    ind_ci_dev[[i]] <- data.frame(t(as.data.frame(mat_mediations_dev[[i]]$d0.ci)))
  } else {
    # Create an empty one-row data frame with NA if the element is not a list/data frame
    ind_ci_dev[[i]] <- data.frame(matrix(NA, nrow = 1, ncol = 1))
  }
  # Assign row name based on `mat_mediations` names
  rownames(ind_ci_dev[[i]]) <- names(mat_mediations_dev)[i]
}

# Assign names to the list
names(ind_ci_dev) <- names(mat_mediations_dev)
...
for (i in 1:length(mat_mediations_dev)) {
  # Check if it's a list/data.frame and contains 'd0'
  if ((is.list(mat_mediations_dev[[i]]) || is.data.frame(mat_mediations_dev[[i]])) && !is.null(mat_mediations_dev[[i]]$d0)) {
    # Assuming 'd0' can be converted directly to a data.frame
    temp_df <- mat_mediations_dev[[i]]$d0
...
names(ind_est_dev) <- names(mat_mediations_dev)
...
nrow(filter(ind_p_est, fdr < 0.05, ind_eff > 0))
nrow(filter(ind_p_est, fdr < 0.05, ind_eff < 0))
...
spour5.0_gff <- as.data.frame(import("~/Documents/GitHub/HotPurps_RNA_04032024_backup/genome_resources/sp5_0_GCF.gff3"))
...
# GO enrichments using Mann Whitney U tests
setwd("~/Documents/GitHub/HotPurps_RNA_04032024_backup/GO_MWU-master/")
...
# Edit these to match your data file names: 
input = "mat_ind_gomwu_new.csv" # two columns of comma-separated values: gene id, continuous measure of significance. To perform standard GO enrichment analysis based on Fisher's exact test, use binary measure (0 or 1, i.e., either sgnificant or not).
